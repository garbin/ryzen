...
const { isFunction, last, capitalize } = require('lodash')
...
class HttpTester {
  ...
  getTest () {
    return async () => {
      ...
      if (data) {
        const body = data instanceof Function ? data() : data
        for (let k in body) {
...
function createGraphQLTester (server, endpoint) {
  const tester = {
    query (query, variables) {
      ...
      graphqlTester.query(query, variables)
  ...
  tester.query.search = (type, fields, variables) => {
    ...
    `, variables).set({ description: `GraphQL: search(${type})` }).assert(res => {
      expect(res.status).toBe(200)
      fields.forEach(field => expect(res.body.data.search.edges[0]).toHaveProperty(`node.${field}`))
    })
  }
  tester.query.fetch = (type, fields, variables) => {
    const name = capitalize(type)
    return tester.query(`
      query ($id: ID!) {
    ...
    `, variables).set({ description: `GraphQL: fetch(${type})` }).assert(res => {
      expect(res.status).toBe(200)
      fields.forEach(field => expect(res.body.data.fetch).toHaveProperty(field))
    })
  }
  tester.mutate.create = (type, variables, fields) => {
    fields = fields || Object.keys((variables instanceof Function ? variables() : variables).input)
    ...
    `, variables).set({ description: `GraphQL: mutation ${mutation}($${type.toLowerCase()})` }).assert(res => {
      ...
      fields.forEach(field =>
        expect(res.body).toHaveProperty(`data.${mutation}.${field}`, variables.input[field])
      )
    })
  }
  tester.mutate.update = (type, variables, fields) => {
    const maybeIncompletedVariables = variables instanceof Function ? variables() : variables
    fields = fields || Object.keys(maybeIncompletedVariables.input)
    type = capitalize(type)
    ...
    `, variables).set({ description: `GraphQL: mutation ${mutation}($${type.toLowerCase()})` }).assert(res => {
      expect(res.status).toBe(200)
      fields.forEach(field =>
        expect(res.body).toHaveProperty(`data.${mutation}.${field}`, maybeIncompletedVariables.input[field])
      )
    })
  }
