...
function createGraphQLTester (server, endpoint) {
  ...
  tester.query.search = ({ type, fields, variables } = {}) => {
    ...
    `, { ...variables, type }).set({ description: `GraphQL: search(${type})` }).assert(res => {
      const firstNode = res.body.data.search.edges[0]
      expect(res.status).toBe(200)
      fields.forEach(field => expect(firstNode).toHaveProperty(`node.${field}`))
    })
  }
  tester.query.fetch = () => {
