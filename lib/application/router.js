...
const { compact, isFunction, isEmpty, defaults, includes, trimStart, last, isArray } = require('lodash')
    ...
    read.list = function () {
      ...
      this.get(this.resourceRootPath, convert(paginate(options.pagination)), compose(middlewares),
        async ctx => {
          ...
          const { filters, q: keywords, sort: orderBy } = ctx.request.query
          ...
          query = query.where(builder => {
            ...
            if (keywords && options.searchable) {
              ...
              builder.where(function () {
                ...
                const applySearch = isArray(options.searchable) ? () => options.searchable.forEach((field, index) => search(field, null, index !== 0)) : options.searchable
