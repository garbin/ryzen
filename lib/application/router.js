...
const { compact, isFunction, isEmpty, defaults, first, includes, trimStart, last, toPairs } = require('lodash')
...
class RESTfulRouter extends Router {
  constructor (model, options = {}) {
    ...
    this.setup = setup => { setup(this); return this }
  }
  ...
  children (...children) {
    this.use(
      ...
      ...children.map(child => {
        assert(child instanceof Router, 'The agruments of children should be instance of Router')
        child.use(async (ctx, next) => {
          const [relationName] = toPairs(this.model.relations).find(([relationName, relationInfo]) => {
            return (relationInfo.modelClass === child.model &&
              [Model.HasManyRelation, Model.HasOneRelation, Model.ManyToManyRelation].includes(relationInfo.relation)
            )
          })
          assert(relationName, new httpErrors.InternalServerError(`Relation not found`))
          ctx.state.query = last(ctx.state.nested).$relatedQuery(relationName)
          await next()
        }).setup(child)
        return child.routes()
      })
...
module.exports = {
  restful (model, setup, children = false) {
    ...
    if (children) {
      router.setup = setup.bind(router, router)
    } else {
      router.setup(setup)
    }
    return router
