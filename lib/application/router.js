...
const { compact, isFunction, isEmpty, defaults, first, includes, trimStart, last, isArray } = require('lodash')
    ...
    read.list = function () {
      ...
      this.get(this.resourceRootPath, convert(paginate(options.pagination)), compose(middlewares),
        async ctx => {
          ...
          query = options.eager ? query.eager(...options.eager) : query
          query = query.where(builder => {
            if (!isEmpty(filters) && options.filterable) {
              const filter = (field, parser) => {
                if (filters[field]) {
                  if (!isFunction(field)) {
                    parser = parser || (value => builder.where({ [field]: value }))
              ...
              const applyFilters = isArray(options.filterable)
                ? () => options.filterable.forEach(filter)
                : options.filterable
            ...
            if (keywords && options.searchable) {
              ...
              builder.where(function () {
                const search = (field, or = false, parser) => {
                  parser = parser || ((keywords, like, query) => query[or ? 'orWhere' : 'where'](field, like, `%${keywords}%`))
                  parser(keywords, like, this)
                }
                const applySearch = isArray(options.searchable) ? () => options.searchable.forEach((field, index) => search(field, index !== 0)) : options.searchable
                applySearch({ search, query: builder, keywords })
              })
            }
            return builder
          })
...
module.exports = {
  restful (model, setup = router => router.crud(), options = {}) {
