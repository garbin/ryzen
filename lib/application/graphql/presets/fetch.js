...
module.exports = function fetch (items, options = {}) {
  ...
  const itemsArray = Object.entries(items).map(([fetchName, fetchConfig]) => {
    const resolve = fetchConfig.resolve || (async (root, { id }, ctx, info) => {
      console.log(fetchConfig, '--------', { [fetchConfig.idColumn || fetchConfig.model.idColumn]: id })
      const item = await fetchConfig.model.query().findOne({ [fetchConfig.idColumn || fetchConfig.model.idColumn]: id })
      return item
    })
    return {
      name: fetchName,
