...
const batch = module.exports = {
  load (options) {
    return async (root, args, ctx, info) => {
      ...
      assert(loader instanceof Loader, 'Can not get loader')
      ...
      const items = await batch.load(Object.assign({}, options, {
        ...
        async fetch (parents) {
          const items = await model.query().where(builder => {
            builder.whereIn(foreignKey, parents.map(parent => parent[parentForeignKey]))
  ...
  belongsToMany (options) {
    return async (root, args, ctx, info) => {
      ...
      const items = await batch.load({
        ...
        assemble (children, parents) {
          return parents.map(parent => children.filter(child => child[relation.foreignKey] === parent[parent.constructor.idColumn]))
        }
      })(root, args, ctx, info)
      return items
    }
    ...
    return async (root, args, ctx, info) => {
      ...
      const items = await batch.load({
        ...
        assemble (items, parents) {
          return parents.map(parent =>
            items.find(item => item[item.constructor.idColumn] === parent[fk]))
        }
      })(root, args, ctx, info)
      return items
    }
